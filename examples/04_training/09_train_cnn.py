...
# The bundled example database uses the legacy Prometheus truth schema,
# which carries `total_energy` (the default regression target).
...
def main(
    ...
    # LCSC spatial size must match this `ImageRepresentation`'s grid. `shape`
    # is [F, D, H, W] per image; LCSC takes the spatial extent (D, H, W) of the
    # single image and prepends the feature count itself.
    _, depth, height, width = image_representation.shape[0]
