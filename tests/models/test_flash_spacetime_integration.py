...
Two weight-identical DeepIce instances — one with `rel_attention="flash"`
— must produce matching per-event cls outputs and parameter gradients.
Both head widths the kernel supports are covered: 48 fills its three
channel chunks, 16 uses one and is the width the reduced model trains at.
...
@pytest.mark.parametrize("head_size", HEAD_SIZES)
def test_deepice_bf16_autocast_parity(head_size: int) -> None:
    """The production training configuration: bf16-mixed autocast."""
    eager, flash = _make_models(7, head_size)
