...
class Detector(Model):
    ...
    def __init__(
        self,
        replace_with_identity: Optional[Union[List[str],str]] = None
    ) -> None:
        """Construct `Detector`.
        ...
        if replace_with_identity is not None:
            ...
            assert isinstance(replace_with_identity,(list)), \
            f"`replace_with_identity` has to be a `list` or `str`."
    ...
    def _standardize(
        ...
        if self._replace_with_identity is not None:
            for feature in self._replace_with_identity:
                ...
                assert feature in feature_map.keys(),\
