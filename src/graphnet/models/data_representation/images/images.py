...
class IC86DNNImage_fast(ImageDefinition):
    """Class creating a image for IC86 DNN data."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if detector is None:
            ...
            assert isinstance(detector, IceCube86)
        node_definition.set_output_feature_names(input_feature_names)
        assert string_label in input_feature_names, f"String label '{string_label}' not in input feature names"
        assert dom_number_label in input_feature_names, f"DOM number label '{dom_number_label}' not in input feature names"

        # Base class constructor
        pixel_mapping = IC86DNNMapping_fast(
