...
def my_sub_sample(
    data, max_length, columns=[0, 1, 2], nb_nearest=8, hlc_pos=6
):
    x = data.x
    btch = data.batch
    ...
    edge_ind = knn_graph(
        x=x[:, columns][node_index], k=nb_nearest, batch=btch[node_index]
    )
...
class flashv2_FourierEncoder(LightningModule):
    ...
    is in the format of (x, y, z, time, charge, auxiliary), the first
    four features being mandatory.
    """
    ...
    def forward(
        ...
        embeddings.append(
            torch.cat(
                [
                    len_emb[i].expand(seq_length[i], -1)
...
class flashv2_attention(LightningModule):
    ...
    def __init__(
        ...
        self.proj_qkv = torch.nn.Linear(
            input_dim, 3 * self.all_head_dim, bias=False, dtype=torch.bfloat16
        )
        if qkv_bias:
        ...
        self.proj = torch.nn.Linear(
            self.all_head_dim, input_dim, dtype=torch.bfloat16
        )
    ...
    def forward(
        ...
        rel_pos_bias: Optional[
            Tensor
        ] = None,  # only here so I can pass the None rel_pos_bias in versions where I don't want that bias
    ) -> Tensor:
        """Forward pass."""
...
class flashv2_attention_unpacked(LightningModule):
    ...
    def __init__(
        ...
        self.proj_q = torch.nn.Linear(
            input_dim, self.all_head_dim, bias=False, dtype=torch.bfloat16
        )
...
class flashv2_block(LightningModule):
    ...
    def __init__(
        ...
        if init_values is not None:
            self.gamma_1 = torch.nn.Parameter(
                init_values * torch.ones(input_dim).to(device=self.device),
                requires_grad=True,
    ...
    def forward(
        ...
        if self.gamma_1 is None:
            ...
            x = x + self.drop_path(
                self.gamma_2.to(device=self.device) * self.mlp(self.norm2(x))
            )
        return x
...
class Theseus_DeepIce(GNN):
    ...
    def __init__(
        ...
        self.rel_pos = dev_flashv2_SpacetimeEncoder(
            head_size, hidden_dim=hidden_dim
        )
        self.sandwich = torch.nn.ModuleList(
            [
                flashv2_block(
                    ...
                    have_rel_bias=have_rel_bias,
                )
                for _ in range(depth_rel)
            ]
        )
        self.cls_token = torch.nn.Linear(
            hidden_dim, 1, bias=False, dtype=torch.bfloat16
        )
    ...
    def forward(self, data: Data) -> Tensor:
        ...
        _, seq_lengths = torch.unique_consecutive(
            batch_v_subsam, return_counts=True
        )
        batch_size = seq_lengths.shape[0]
        cu_seqs = torch.nn.functional.pad(
            seq_lengths.cumsum(0), pad=(1, 0), value=0
        ).to(torch.int32)
        max_seq = (
            seq_lengths.max().to(device=self.device, dtype=torch.int32).item()
        )
        ...
        # insert cls_token
        cls_token = self.cls_token.weight.expand(batch_size, -1)
        # create empty tensor and append cls_tokens at the beginning of the individual sequences; fill the rest with original x
        emp = torch.empty((x.shape[0] + batch_size, x.shape[1])).to(
            device=self.device, dtype=torch.bfloat16
        )
        cls_ind = cu_seqs + torch.arange(batch_size + 1).to(
...
def main(
    ...
    transformer = Theseus_DeepIce(
        seq_length=2560, n_features=6, have_rel_bias=True
    )
